...
const generateRoast = async ({ apiKey, target, profileSummary, context, authorName }) => {
    ...
    if (!roast) throw new Error('NVIDIA roast returned an invalid response')
...
module.exports = {
    ...
    run: async (client, message, args) => {
        const apiKey = process.env.NVIDIA_API_KEY || client.config.NVIDIA_API_KEY
            ...
            return message.channel.send({
                embeds: [
                    client.util.embed()
                        ...
                        .setDescription(`${client.emoji.cross} | Missing \`NVIDIA_API_KEY\`. Add it to your \`.env\` file and restart the bot.`)
                ]
            })
        }
        ...
        try {
            ...
            const embed = client.util.embed()
                ...
                .setFooter({
                    text: 'akashsuu roast • NVIDIA AI vision',
